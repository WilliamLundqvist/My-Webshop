...
export const processCategories = (categoryData: any) => {
  ...
  const processedCategories = categoryData.productCategory.children.nodes.map((category) => {
  ...
  return processedCategories.filter((category) => category.children.nodes.length > 0);
};
